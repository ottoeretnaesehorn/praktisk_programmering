...
void create_and_print_spline (int n, double x[], double y[])
{
	...
	gsl_interp * gsl_cspline = gsl_interp_alloc (gsl_interp_cspline, n); 
	gsl_interp_init (gsl_cspline, x, y, n); 
	...
	for (double z = x[0]; z <= x[n - 1]; z += inc)
	{
		fprintf (spline_data, 
				...
				gsl_interp_eval (gsl_cspline, x, y, z, NULL), 
				cubic_spline_integ (spline, z),
				gsl_interp_eval_integ (gsl_cspline, x, y, x[0], z, NULL),
				cubic_spline_deriv (spline, z),
				gsl_interp_eval_deriv (gsl_cspline, x, y, z, NULL));
	}

	cubic_spline_free (spline);
	gsl_interp_free (gsl_cspline);
	fclose (spline_data); 
}
